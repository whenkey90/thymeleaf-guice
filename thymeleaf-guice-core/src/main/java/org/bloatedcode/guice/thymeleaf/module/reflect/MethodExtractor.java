...
public class MethodExtractor {
	...
	public Method findMethod(String name, Class<?> clazz) {
		...
		for (Method method : clazz.getMethods()) {
			if (method.getName().equals(name) && foundMethod != null) {
				throw new ThymeleafInitializationException("More than one method with name '" + name + "', please specify the parameters.");
			}
		...
		if (foundMethod == null) {
			throw new ThymeleafInitializationException("No method with name '" + name + "'");
		}
	...
	public Method findMethod(String name, Class<?> clazz, Class<?>... parameters) {
		...
		try {
			...
			throw new ThymeleafInitializationException("No method with name '" + name + "' and arguments '" + parameters.toString() + "'", e);
		}
		return findMethod;
	}
